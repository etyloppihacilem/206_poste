...
void init_matrix(void) {
    ...
    LPC_TIM3->MCR  = (1 << 0) | (1 << 1) | (1 << 2); // Interruption et reset sur MR0
...
void register_request(void) {
    ...
    /* --- gestion anti-rebond et répétition ------------------------ */
    if (!key) {       /* aucune touche pressée             */
        last_key = 0; /* ← prêt à accepter la même touche  */
        return;
    }
    if (key == last_key) /* touche toujours maintenue         */
        return;          /* on ignore les répétitions          */

    last_key      = key;     
    rebond        = 0;        
    LPC_TIM3->TCR = (1 << 1); //reset timer 
    LPC_TIM3->TCR = (1 << 0); // restart timer

    /* --- constitution de la requête ------------------------------- */
    if (idx == 0 && key != '#')
        return; 

    if (idx < SIZE_REQUEST - 1)
        request[idx++] = key;
...
void DIP_switch() {
    ...
    num_poste = dip_switch_value;
}

/*
 * returns message to treat, or NULL
 * */
message_pave *get_pave_msg(void) {
    if (FIFO_r == FIFO_w)
        return 0;
    return &FIFO_pave[FIFO_r];
}
